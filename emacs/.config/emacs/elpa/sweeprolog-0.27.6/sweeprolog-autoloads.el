...
`Sweeprolog-Electric-Layout mode' mode.  If the prefix argument is
...
evaluate the variable `sweeprolog-electric-layout-mode'.
...
This is a minor mode.  If called interactively, toggle the
`Sweeprolog-Forward-Hole-On-Tab mode' mode.  If the prefix argument is
positive, enable the mode, and if it is zero or negative, disable the
mode.
...
To check whether the minor mode is enabled in the current buffer,
evaluate the variable `sweeprolog-forward-hole-on-tab-mode'.
